...
/*   Updated: 2025/03/22 17:01:03 by falakus          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	character_check(t_game *game)
{
	...
	while (i < game->map_height)
	{
        ...
        {
            if (game->map[i][j] != '1' && game->map[i][j] != '0' && game->map[i][j] != 'P' && game->map[i][j] != 'E' && game->map[i][j] != 'C' && game->map[i][j] != '\n')
	...
	if (game->player != 1 || game->exit != 1 || game->coins == 0)
		print_error (RED"The map must contain exactly one player, one exit, and at least one coin!"RESET,game);
}
